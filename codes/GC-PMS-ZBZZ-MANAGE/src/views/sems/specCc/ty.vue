<template>
  <div class="app-container">
    ...
    <el-table ref="deviceTable" v-loading="loading" :cell-style="tableCellStyle" :data="specCcList"
      ...
      <el-table-column label="型号" :show-overflow-tooltip="true" align="center" prop="model"/>
      <el-table-column label="使用登记证编号 车牌号" :show-overflow-tooltip="true" align="center" fixed="left"
                       prop="carNo" width="180"/>
      <el-table-column label="设备代码" :show-overflow-tooltip="true" align="center" prop="regno"/>
      <el-table-column label="发动机号/底盘号" :show-overflow-tooltip="true" align="center" prop="frameNo"
                       width="150"/>
    ...
    <el-dialog :close-on-click-modal="false" v-dialogDrag :title="title" :visible.sync="open" append-to-body
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="140px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="使用登记证编号/车牌号" prop="carNo">
              <el-input v-model="form.carNo" placeholder="请输入使用登记证编号/车牌号"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="使用场所" prop="useDept">
              <el-input v-model="form.useDept" placeholder="请输入使用场所"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="设备代码" prop="regno">
              <el-input v-model="form.regno" placeholder="请输入设备代码"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="发动机号/底盘号" prop="frameNo">
              <el-input v-model="form.frameNo" placeholder="请输入车架（底盘）编号"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="防爆级别" prop="exGrade">
              <el-input v-model="form.exGrade" placeholder="请输入防爆级别"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = '添加特种设备场内机动车辆台账';
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getSpecCc(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = '修改特种设备场内机动车辆台账';
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    addAprrove(row, type) {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (rows[i].assetOwner != '自主产权') {
          this.$alert('当前设备非自主产权，无法申请', '提示', {
            type: 'warning'
          })
          return
        }
